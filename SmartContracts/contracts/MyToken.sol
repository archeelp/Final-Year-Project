...
/// @custom:security-contact adityapatkar22@gmail.com
contract MyToken is ERC1155, AccessControl, ERC1155Supply {
    ...
    event AddNewToken(address tokenAdmin, uint256 tokenID, uint256 amount, uint256 rate);
    event BuyToken(address tokenAdmin, address buyer, uint256 tokenID, uint256 amount, bytes data);
    event Disburse(address tokenAdmin, address holder, uint256 tokenID, bool success, bytes data, uint256 amount);
    ...
    modifier tokenExists(uint256 tokenID) {
        require(_tokens[tokenID].tokenAdmin != address(0), "Token Does Not Exist");
        _;
    }
    ...
    // Adding a new token 
    function addNewToken(address payable tokenAdmin, uint256 amount, uint256 rate) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function buyToken(uint256 tokenID) public payable tokenExists(tokenID) {
        (bool success, bytes memory data) = _tokens[tokenID].tokenAdmin.call{value: msg.value}("");
        require(success,"Transaction failed!");
        uint256 amount = msg.value/_tokens[tokenID].rate;
        _safeTransferFrom(_tokens[tokenID].tokenAdmin,msg.sender,tokenID,amount,data);
        emit BuyToken(_tokens[tokenID].tokenAdmin,msg.sender,tokenID,amount,data);
    }

    // Disburse to holders 
    function disburse(uint256 tokenID) public payable isTokenAdmin(tokenID) {
        for(uint256 i=0;i<_tokenHolders.length;i++){
            uint256 amount = balanceOf(_tokenHolders[i],tokenID)*msg.value/totalSupply(tokenID);
            (bool success, bytes memory data) = _tokenHolders[i].call{value: amount}("");
            emit Disburse(_tokens[tokenID].tokenAdmin,_tokenHolders[i],tokenID,success,data,amount);
        }
    }

    function createPoll(uint256 tokenID,string memory question, string[] memory options) public isTokenAdmin(tokenID) {
    ...
    function vote(uint256 tokenID, uint256 pollID, uint256 option) public notVoted(tokenID,pollID) {
    ...
    function getVoteResults(uint256 tokenID,uint256 pollID) public view returns(string memory,uint256){
