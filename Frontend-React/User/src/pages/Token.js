    ...
    const init = async () => {
      ...
      try {
        ...
        SC.init().then(async () => {
          ...
          setPolls(polls[0].map((poll, pollIndex) => {
            return {
        ...
        <div className="container px-5 py-24 mx-auto">
          <div className="lg:w-4/5 mx-auto flex flex-wrap">
            ...
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
              <h2 className="text-sm title-font text-gray-500 tracking-widest">TOKEN NAME</h2>
              <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{token.name?.toUpperCase()} TOKEN</h1>
              <div className="flex mb-1 w-50">
                <span className="flex items-center">
                  Percent Raised 25%
                  <div className="w-40 bg-gray-200 h-2 ml-2">
                    <div className="bg-indigo-600 h-2" style={{ width: "25%" }}></div>
                  </div>
                </span>
              </div>
              <div className="flex mb-1 w-50">
                <span className="text-gray-600">Total Supply: {token.amount} {token.name?.toUpperCase()} TOKEN</span>
              </div>
              <div className="flex mb-4">
                <h3 className="text-l font-medium flex items-center text-indigo-600">1 {token.name?.toUpperCase()} TOKEN = {token.rate / oneETH} ETH</h3>
              </div>
              ...
              </div>
              {
                polls.length > 0 && (
                  <div className="flex">
                    <ArrowLeftIcon className="w-10 h-10 flex-none m-auto" onClick={() => setPollsIndex((pollsIndex - 1) % polls.length)} />
                    <div className="w-80 mt-5 m-auto lg:mt-10 max-w-sm grow">
                      <div className="bg-white shadow-2xl rounded-b-3xl pb-4">
                        <h2 className="text-center text-gray-800 text-2xl font-bold pt-6">Poll</h2>
                        <div className="w-5/6 m-auto">
                          <p className="text-center text-gray-500 pt-5">{polls[pollsIndex]?.question}</p>
                        </div>
                        <div className="grid grid-cols-4 w-72 lg:w-5/6 m-auto bg-indigo-50 mt-5 p-4 lg:p-4 rounded-2xl">
                          ...
                          <div className="col-span-2 pt-1">
                            <p className="text-gray-800 font-bold lg:text-l mb-1">Result: {polls[pollsIndex].result}</p>
                            <span className="flex items-center">
                            ...
                            <div className="flex items-center mr-5">
                              <span className="mr-3 text-gray-500 text-sm">Options</span>
                              <div className="relative">
                                <select onChange={(e) => setOptionIndex(e.target.value)} className="rounded border appearance-none border-gray-300 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 text-base pl-3 pr-10">
                                  {polls[pollsIndex]?.options.map((option, optionIndex) => {
                                    return <option value={optionIndex} key={optionIndex}>{option}</option>
                                  })}
                                </select>
                        ...
                        <div className="bg-indigo-700 w-72 lg:w-5/6 text-center m-auto mt-6 p-2 hover:bg-indigo-500 rounded-2xl text-white text-center shadow-xl shadow-bg-blue-700">
                          <button className="lg:text-sm text-lg font-bold" onClick={vote}>Submit Vote</button>
                        </div>
                      </div>
                    </div>
                    <ArrowRightIcon className="w-10 h-10 flex-none m-auto" onClick={() => setPollsIndex((pollsIndex + 1) % polls.length)} />
                  </div>
                )
              }
            </div>
